...
In order to choose the solution which best represents the two top quarks in a $t\bar{t}$ system, the likelihood of each solution is evaluated in the SM $t\bar{t}$ hypothesis. This is performed using the product of probabilities derived from certain distributions of variables from simulated $t\bar{t}$ events.\\
...
\begin{equation}
\mu (tWZ) = \frac{\sigma_{exp}(tWZ)}{\sigma_{SM}(tWZ)}
\end{equation}
where $\sigma_{exp}(tWZ)$ expected cross section of $tWZ$ and $\sigma_{SM}(tWZ)$ is the SM cross section of $tWZ$. A value of $\mu (tWZ) = 0$ represents the background-only hypothesis, where no $tWZ$ is present. A value of $\mu (tWZ) = 1$ represents the signal$+$background hypothesis, where the expected $tWZ$ signal is exactly the SM prediction.

A fit across all regions in the tetralepton channel is performed to determine the sensitivity $tWZ$ in this channel. Furthermore, a combined fit is performed to another entirely independent analysis that uses separate data and simulation events in the trilepton channel
